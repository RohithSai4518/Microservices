/**
 * InventoryThreshold - Domain Model & Invariant Engine
...
class InventoryThreshold extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('InventoryThreshold validation error: ID must be populated');
    if (!this.domain) throw new Error('InventoryThreshold validation error: Domain context required');
    return true;
  }
