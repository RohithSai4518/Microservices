/**
 * StockReservation - Domain Model & Invariant Engine
...
class StockReservation extends AggregateRoot {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('StockReservation validation error: ID must be populated');
    if (!this.domain) throw new Error('StockReservation validation error: Domain context required');
    return true;
  }
