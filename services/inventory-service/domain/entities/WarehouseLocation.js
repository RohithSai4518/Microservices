/**
 * WarehouseLocation - Domain Model & Invariant Engine
...
class WarehouseLocation extends AggregateRoot {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('WarehouseLocation validation error: ID must be populated');
    if (!this.domain) throw new Error('WarehouseLocation validation error: Domain context required');
    return true;
  }
