/**
 * SupplierCatalogItem - Domain Model & Invariant Engine
...
class SupplierCatalogItem extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('SupplierCatalogItem validation error: ID must be populated');
    if (!this.domain) throw new Error('SupplierCatalogItem validation error: Domain context required');
    return true;
  }
