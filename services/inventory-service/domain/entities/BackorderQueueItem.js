/**
 * BackorderQueueItem - Domain Model & Invariant Engine
...
class BackorderQueueItem extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('BackorderQueueItem validation error: ID must be populated');
    if (!this.domain) throw new Error('BackorderQueueItem validation error: Domain context required');
    return true;
  }
...
module.exports = { BackorderQueueItem };
