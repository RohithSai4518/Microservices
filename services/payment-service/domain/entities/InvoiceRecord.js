/**
 * InvoiceRecord - Domain Model & Invariant Engine
...
class InvoiceRecord extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('InvoiceRecord validation error: ID must be populated');
    if (!this.domain) throw new Error('InvoiceRecord validation error: Domain context required');
    return true;
  }
