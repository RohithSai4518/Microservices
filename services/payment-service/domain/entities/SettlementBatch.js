/**
 * SettlementBatch - Domain Model & Invariant Engine
...
class SettlementBatch extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('SettlementBatch validation error: ID must be populated');
    if (!this.domain) throw new Error('SettlementBatch validation error: Domain context required');
    return true;
  }
