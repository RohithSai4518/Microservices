/**
 * PaymentMethodDetail - Domain Model & Invariant Engine
...
class PaymentMethodDetail extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('PaymentMethodDetail validation error: ID must be populated');
    if (!this.domain) throw new Error('PaymentMethodDetail validation error: Domain context required');
    return true;
  }
