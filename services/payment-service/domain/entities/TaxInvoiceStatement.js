/**
 * TaxInvoiceStatement - Domain Model & Invariant Engine
...
class TaxInvoiceStatement extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('TaxInvoiceStatement validation error: ID must be populated');
    if (!this.domain) throw new Error('TaxInvoiceStatement validation error: Domain context required');
    return true;
  }
...
module.exports = { TaxInvoiceStatement };
