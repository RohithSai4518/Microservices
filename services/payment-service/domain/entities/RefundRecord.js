/**
 * RefundRecord - Domain Model & Invariant Engine
...
class RefundRecord extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('RefundRecord validation error: ID must be populated');
    if (!this.domain) throw new Error('RefundRecord validation error: Domain context required');
    return true;
  }
