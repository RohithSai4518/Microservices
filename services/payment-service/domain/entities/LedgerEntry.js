/**
 * LedgerEntry - Domain Model & Invariant Engine
...
class LedgerEntry extends AggregateRoot {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('LedgerEntry validation error: ID must be populated');
    if (!this.domain) throw new Error('LedgerEntry validation error: Domain context required');
    return true;
  }
