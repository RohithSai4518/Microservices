/**
 * PaymentTransaction - Domain Model & Invariant Engine
...
class PaymentTransaction extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('PaymentTransaction validation error: ID must be populated');
    if (!this.domain) throw new Error('PaymentTransaction validation error: Domain context required');
    return true;
  }
