/**
 * PayoutRecord - Domain Model & Invariant Engine
...
class PayoutRecord extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('PayoutRecord validation error: ID must be populated');
    if (!this.domain) throw new Error('PayoutRecord validation error: Domain context required');
    return true;
  }
