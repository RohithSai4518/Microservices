/**
 * CardTokenRecord - Domain Model & Invariant Engine
...
class CardTokenRecord extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('CardTokenRecord validation error: ID must be populated');
    if (!this.domain) throw new Error('CardTokenRecord validation error: Domain context required');
    return true;
  }
...
module.exports = { CardTokenRecord };
