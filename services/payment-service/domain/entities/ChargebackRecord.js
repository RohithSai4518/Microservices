/**
 * ChargebackRecord - Domain Model & Invariant Engine
...
class ChargebackRecord extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('ChargebackRecord validation error: ID must be populated');
    if (!this.domain) throw new Error('ChargebackRecord validation error: Domain context required');
    return true;
  }
...
module.exports = { ChargebackRecord };
