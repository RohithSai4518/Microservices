/**
 * TaxCalculationService - Application Service
...
class TaxCalculationService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('payment-taxcalculationservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in TaxCalculationService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('payment.taxcalculationservice.executed', {
      ...
      this.logger.error('Error executing TaxCalculationService: ' + err.message, { traceId });
