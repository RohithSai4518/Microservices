/**
 * FraudDetectionService - Application Service
...
class FraudDetectionService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('payment-frauddetectionservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in FraudDetectionService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('payment.frauddetectionservice.executed', {
      ...
      this.logger.error('Error executing FraudDetectionService: ' + err.message, { traceId });
