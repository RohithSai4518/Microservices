/**
 * DoubleEntryLedgerService - Application Service
...
class DoubleEntryLedgerService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('payment-doubleentryledgerservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in DoubleEntryLedgerService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      const computationResult = {
        ...
        service: 'DoubleEntryLedgerService',
        status: 'PROCESSED',
      ...
      if (this.eventBus) {
        await this.eventBus.publish('payment.doubleentryledgerservice.executed', {
      ...
      this.logger.error('Error executing DoubleEntryLedgerService: ' + err.message, { traceId });
...
module.exports = { DoubleEntryLedgerService };
