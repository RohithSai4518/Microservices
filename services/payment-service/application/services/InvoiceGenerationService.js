/**
 * InvoiceGenerationService - Application Service
...
class InvoiceGenerationService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('payment-invoicegenerationservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in InvoiceGenerationService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('payment.invoicegenerationservice.executed', {
      ...
      this.logger.error('Error executing InvoiceGenerationService: ' + err.message, { traceId });
