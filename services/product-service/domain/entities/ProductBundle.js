/**
 * ProductBundle - Domain Model & Invariant Engine
...
class ProductBundle extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('ProductBundle validation error: ID must be populated');
    if (!this.domain) throw new Error('ProductBundle validation error: Domain context required');
    return true;
  }
