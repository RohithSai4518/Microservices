/**
 * ProductCategory - Domain Model & Invariant Engine
...
class ProductCategory extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('ProductCategory validation error: ID must be populated');
    if (!this.domain) throw new Error('ProductCategory validation error: Domain context required');
    return true;
  }
