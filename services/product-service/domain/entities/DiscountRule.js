/**
 * DiscountRule - Domain Model & Invariant Engine
...
class DiscountRule extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('DiscountRule validation error: ID must be populated');
    if (!this.domain) throw new Error('DiscountRule validation error: Domain context required');
    return true;
  }
