/**
 * ProductVariant - Domain Model & Invariant Engine
...
class ProductVariant extends AggregateRoot {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('ProductVariant validation error: ID must be populated');
    if (!this.domain) throw new Error('ProductVariant validation error: Domain context required');
    return true;
  }
