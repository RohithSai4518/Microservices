/**
 * PriceTier - Domain Model & Invariant Engine
...
class PriceTier extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('PriceTier validation error: ID must be populated');
    if (!this.domain) throw new Error('PriceTier validation error: Domain context required');
    return true;
  }
