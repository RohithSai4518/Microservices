/**
 * ProductReview - Domain Model & Invariant Engine
...
class ProductReview extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('ProductReview validation error: ID must be populated');
    if (!this.domain) throw new Error('ProductReview validation error: Domain context required');
    return true;
  }
