/**
 * ProductRecommendation - Domain Model & Invariant Engine
...
class ProductRecommendation extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('ProductRecommendation validation error: ID must be populated');
    if (!this.domain) throw new Error('ProductRecommendation validation error: Domain context required');
    return true;
  }
