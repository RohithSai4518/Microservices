/**
 * ProductBrand - Domain Model & Invariant Engine
...
class ProductBrand extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('ProductBrand validation error: ID must be populated');
    if (!this.domain) throw new Error('ProductBrand validation error: Domain context required');
    return true;
  }
