/**
 * ProductSeoMetadata - Domain Model & Invariant Engine
...
class ProductSeoMetadata extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('ProductSeoMetadata validation error: ID must be populated');
    if (!this.domain) throw new Error('ProductSeoMetadata validation error: Domain context required');
    return true;
  }
...
module.exports = { ProductSeoMetadata };
