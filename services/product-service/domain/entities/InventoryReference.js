/**
 * InventoryReference - Domain Model & Invariant Engine
...
class InventoryReference extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('InventoryReference validation error: ID must be populated');
    if (!this.domain) throw new Error('InventoryReference validation error: Domain context required');
    return true;
  }
