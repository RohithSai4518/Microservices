/**
 * ProductAttribute - Domain Model & Invariant Engine
...
class ProductAttribute extends AggregateRoot {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('ProductAttribute validation error: ID must be populated');
    if (!this.domain) throw new Error('ProductAttribute validation error: Domain context required');
    return true;
  }
