/**
 * TaxCategory - Domain Model & Invariant Engine
...
class TaxCategory extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('TaxCategory validation error: ID must be populated');
    if (!this.domain) throw new Error('TaxCategory validation error: Domain context required');
    return true;
  }
