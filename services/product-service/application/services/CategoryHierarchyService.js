/**
 * CategoryHierarchyService - Application Service
...
class CategoryHierarchyService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('product-categoryhierarchyservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in CategoryHierarchyService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('product.categoryhierarchyservice.executed', {
      ...
      this.logger.error('Error executing CategoryHierarchyService: ' + err.message, { traceId });
...
module.exports = { CategoryHierarchyService };
