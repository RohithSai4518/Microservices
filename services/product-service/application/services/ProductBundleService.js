/**
 * ProductBundleService - Application Service
...
class ProductBundleService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('product-productbundleservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in ProductBundleService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('product.productbundleservice.executed', {
      ...
      this.logger.error('Error executing ProductBundleService: ' + err.message, { traceId });
