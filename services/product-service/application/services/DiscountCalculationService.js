/**
 * DiscountCalculationService - Application Service
...
class DiscountCalculationService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('product-discountcalculationservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in DiscountCalculationService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('product.discountcalculationservice.executed', {
      ...
      this.logger.error('Error executing DiscountCalculationService: ' + err.message, { traceId });
