/**
 * ExecutiveReportService - Application Service
...
class ExecutiveReportService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('analytics-executivereportservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in ExecutiveReportService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('analytics.executivereportservice.executed', {
      ...
      this.logger.error('Error executing ExecutiveReportService: ' + err.message, { traceId });
