/**
 * AovTrendRecord - Domain Model & Invariant Engine
...
class AovTrendRecord extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('AovTrendRecord validation error: ID must be populated');
    if (!this.domain) throw new Error('AovTrendRecord validation error: Domain context required');
    return true;
  }
