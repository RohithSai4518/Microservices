/**
 * LatencySample - Domain Model & Invariant Engine
...
class LatencySample extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('LatencySample validation error: ID must be populated');
    if (!this.domain) throw new Error('LatencySample validation error: Domain context required');
    return true;
  }
