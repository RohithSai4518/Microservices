/**
 * TelemetryEventRecord - Domain Model & Invariant Engine
...
class TelemetryEventRecord extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('TelemetryEventRecord validation error: ID must be populated');
    if (!this.domain) throw new Error('TelemetryEventRecord validation error: Domain context required');
    return true;
  }
...
module.exports = { TelemetryEventRecord };
