/**
 * DistributedTraceSpan - Domain Model & Invariant Engine
...
class DistributedTraceSpan extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('DistributedTraceSpan validation error: ID must be populated');
    if (!this.domain) throw new Error('DistributedTraceSpan validation error: Domain context required');
    return true;
  }
...
module.exports = { DistributedTraceSpan };
