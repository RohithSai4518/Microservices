/**
 * PasswordHistory - Domain Model & Invariant Engine
...
class PasswordHistory extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('PasswordHistory validation error: ID must be populated');
    if (!this.domain) throw new Error('PasswordHistory validation error: Domain context required');
    return true;
  }
