/**
 * AuthTokenRecord - Domain Model & Invariant Engine
...
class AuthTokenRecord extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('AuthTokenRecord validation error: ID must be populated');
    if (!this.domain) throw new Error('AuthTokenRecord validation error: Domain context required');
    return true;
  }
...
module.exports = { AuthTokenRecord };
