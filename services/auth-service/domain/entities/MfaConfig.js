/**
 * MfaConfig - Domain Model & Invariant Engine
...
class MfaConfig extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('MfaConfig validation error: ID must be populated');
    if (!this.domain) throw new Error('MfaConfig validation error: Domain context required');
    return true;
  }
