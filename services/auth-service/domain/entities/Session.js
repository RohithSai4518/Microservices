/**
 * Session - Domain Model & Invariant Engine
...
class Session extends AggregateRoot {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('Session validation error: ID must be populated');
    if (!this.domain) throw new Error('Session validation error: Domain context required');
    return true;
  }
