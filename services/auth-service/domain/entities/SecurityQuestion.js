/**
 * SecurityQuestion - Domain Model & Invariant Engine
...
class SecurityQuestion extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('SecurityQuestion validation error: ID must be populated');
    if (!this.domain) throw new Error('SecurityQuestion validation error: Domain context required');
    return true;
  }
