/**
 * ApiKeyCredential - Domain Model & Invariant Engine
...
class ApiKeyCredential extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('ApiKeyCredential validation error: ID must be populated');
    if (!this.domain) throw new Error('ApiKeyCredential validation error: Domain context required');
    return true;
  }
...
module.exports = { ApiKeyCredential };
