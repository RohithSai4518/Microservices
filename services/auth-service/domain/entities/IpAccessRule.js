/**
 * IpAccessRule - Domain Model & Invariant Engine
...
class IpAccessRule extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('IpAccessRule validation error: ID must be populated');
    if (!this.domain) throw new Error('IpAccessRule validation error: Domain context required');
    return true;
  }
