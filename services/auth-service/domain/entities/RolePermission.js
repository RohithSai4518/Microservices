/**
 * RolePermission - Domain Model & Invariant Engine
...
class RolePermission extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('RolePermission validation error: ID must be populated');
    if (!this.domain) throw new Error('RolePermission validation error: Domain context required');
    return true;
  }
