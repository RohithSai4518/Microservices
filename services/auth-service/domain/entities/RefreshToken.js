/**
 * RefreshToken - Domain Model & Invariant Engine
...
class RefreshToken extends AggregateRoot {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('RefreshToken validation error: ID must be populated');
    if (!this.domain) throw new Error('RefreshToken validation error: Domain context required');
    return true;
  }
