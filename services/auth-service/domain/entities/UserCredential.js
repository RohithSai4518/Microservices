/**
 * UserCredential - Domain Model & Invariant Engine
...
class UserCredential extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('UserCredential validation error: ID must be populated');
    if (!this.domain) throw new Error('UserCredential validation error: Domain context required');
    return true;
  }
