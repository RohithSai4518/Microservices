/**
 * OAuthProviderLink - Domain Model & Invariant Engine
...
class OAuthProviderLink extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('OAuthProviderLink validation error: ID must be populated');
    if (!this.domain) throw new Error('OAuthProviderLink validation error: Domain context required');
    return true;
  }
...
module.exports = { OAuthProviderLink };
