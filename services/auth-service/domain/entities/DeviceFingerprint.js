/**
 * DeviceFingerprint - Domain Model & Invariant Engine
...
class DeviceFingerprint extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('DeviceFingerprint validation error: ID must be populated');
    if (!this.domain) throw new Error('DeviceFingerprint validation error: Domain context required');
    return true;
  }
