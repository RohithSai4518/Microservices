/**
 * DeviceTrackingService - Application Service
...
class DeviceTrackingService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('auth-devicetrackingservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in DeviceTrackingService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('auth.devicetrackingservice.executed', {
      ...
      this.logger.error('Error executing DeviceTrackingService: ' + err.message, { traceId });
...
module.exports = { DeviceTrackingService };
