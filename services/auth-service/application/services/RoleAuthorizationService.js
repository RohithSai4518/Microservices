/**
 * RoleAuthorizationService - Application Service
...
class RoleAuthorizationService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('auth-roleauthorizationservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in RoleAuthorizationService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('auth.roleauthorizationservice.executed', {
      ...
      this.logger.error('Error executing RoleAuthorizationService: ' + err.message, { traceId });
