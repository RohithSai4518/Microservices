/**
 * SessionManagerService - Application Service
...
class SessionManagerService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('auth-sessionmanagerservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in SessionManagerService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('auth.sessionmanagerservice.executed', {
      ...
      this.logger.error('Error executing SessionManagerService: ' + err.message, { traceId });
