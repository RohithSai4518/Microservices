/**
 * TokenManagementService - Application Service
...
class TokenManagementService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('auth-tokenmanagementservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in TokenManagementService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('auth.tokenmanagementservice.executed', {
      ...
      this.logger.error('Error executing TokenManagementService: ' + err.message, { traceId });
