...
class AuthenticationService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('auth-authenticationservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in AuthenticationService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('auth.authenticationservice.executed', {
      ...
      this.logger.error('Error executing AuthenticationService: ' + err.message, { traceId });
