/**
 * OrderNotificationCoordinator - Application Service
...
class OrderNotificationCoordinator {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('order-ordernotificationcoordinator');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in OrderNotificationCoordinator', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      const computationResult = {
        ...
        service: 'OrderNotificationCoordinator',
        status: 'PROCESSED',
      ...
      if (this.eventBus) {
        await this.eventBus.publish('order.ordernotificationcoordinator.executed', {
      ...
      this.logger.error('Error executing OrderNotificationCoordinator: ' + err.message, { traceId });
...
module.exports = { OrderNotificationCoordinator };
