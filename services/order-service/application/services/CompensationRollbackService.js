/**
 * CompensationRollbackService - Application Service
...
class CompensationRollbackService {
  constructor(repository, eventBus, options = {}) {
    this.repository = repository;
    this.eventBus = eventBus;
    this.options = options;
    this.logger = new Logger('order-compensationrollbackservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in CompensationRollbackService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      const computationResult = {
        ...
        service: 'CompensationRollbackService',
        status: 'PROCESSED',
      ...
      if (this.eventBus) {
        await this.eventBus.publish('order.compensationrollbackservice.executed', {
      ...
      this.logger.error('Error executing CompensationRollbackService: ' + err.message, { traceId });
...
module.exports = { CompensationRollbackService };
