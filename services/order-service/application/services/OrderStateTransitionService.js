/**
 * OrderStateTransitionService - Application Service
...
class OrderStateTransitionService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('order-orderstatetransitionservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in OrderStateTransitionService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      const computationResult = {
        ...
        service: 'OrderStateTransitionService',
        status: 'PROCESSED',
      ...
      if (this.eventBus) {
        await this.eventBus.publish('order.orderstatetransitionservice.executed', {
      ...
      this.logger.error('Error executing OrderStateTransitionService: ' + err.message, { traceId });
