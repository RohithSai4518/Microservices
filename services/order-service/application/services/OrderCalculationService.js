/**
 * OrderCalculationService - Application Service
...
class OrderCalculationService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('order-ordercalculationservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in OrderCalculationService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('order.ordercalculationservice.executed', {
      ...
      this.logger.error('Error executing OrderCalculationService: ' + err.message, { traceId });
