/**
 * OrderSplitterService - Application Service
...
class OrderSplitterService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('order-ordersplitterservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in OrderSplitterService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('order.ordersplitterservice.executed', {
      ...
      this.logger.error('Error executing OrderSplitterService: ' + err.message, { traceId });
