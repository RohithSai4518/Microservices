/**
 * OrderNote - Domain Model & Invariant Engine
...
class OrderNote extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('OrderNote validation error: ID must be populated');
    if (!this.domain) throw new Error('OrderNote validation error: Domain context required');
    return true;
  }
