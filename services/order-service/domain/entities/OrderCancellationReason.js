/**
 * OrderCancellationReason - Domain Model & Invariant Engine
...
class OrderCancellationReason extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('OrderCancellationReason validation error: ID must be populated');
    if (!this.domain) throw new Error('OrderCancellationReason validation error: Domain context required');
    return true;
  }
