/**
 * OrderItemRecord - Domain Model & Invariant Engine
...
class OrderItemRecord extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('OrderItemRecord validation error: ID must be populated');
    if (!this.domain) throw new Error('OrderItemRecord validation error: Domain context required');
    return true;
  }
...
module.exports = { OrderItemRecord };
