/**
 * CompensationActionLog - Domain Model & Invariant Engine
...
class CompensationActionLog extends AggregateRoot {
  constructor(props = {}) {
    super(props.id || ('ord_' + Date.now().toString(36) + '_' + Math.random().toString(36).substring(2, 8)));
    this.domain = 'order';
  ...
  validateInvariants() {
    if (!this.id) throw new Error('CompensationActionLog validation error: ID must be populated');
    if (!this.domain) throw new Error('CompensationActionLog validation error: Domain context required');
    return true;
  }
...
module.exports = { CompensationActionLog };
