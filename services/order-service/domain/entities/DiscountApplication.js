/**
 * DiscountApplication - Domain Model & Invariant Engine
...
class DiscountApplication extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('DiscountApplication validation error: ID must be populated');
    if (!this.domain) throw new Error('DiscountApplication validation error: Domain context required');
    return true;
  }
