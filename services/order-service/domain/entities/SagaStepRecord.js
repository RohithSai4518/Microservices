/**
 * SagaStepRecord - Domain Model & Invariant Engine
...
class SagaStepRecord extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('SagaStepRecord validation error: ID must be populated');
    if (!this.domain) throw new Error('SagaStepRecord validation error: Domain context required');
    return true;
  }
