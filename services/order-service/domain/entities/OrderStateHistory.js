/**
 * OrderStateHistory - Domain Model & Invariant Engine
...
class OrderStateHistory extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('OrderStateHistory validation error: ID must be populated');
    if (!this.domain) throw new Error('OrderStateHistory validation error: Domain context required');
    return true;
  }
...
module.exports = { OrderStateHistory };
