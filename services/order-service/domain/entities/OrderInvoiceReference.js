/**
 * OrderInvoiceReference - Domain Model & Invariant Engine
...
class OrderInvoiceReference extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('OrderInvoiceReference validation error: ID must be populated');
    if (!this.domain) throw new Error('OrderInvoiceReference validation error: Domain context required');
    return true;
  }
...
module.exports = { OrderInvoiceReference };
