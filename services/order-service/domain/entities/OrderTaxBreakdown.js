/**
 * OrderTaxBreakdown - Domain Model & Invariant Engine
...
class OrderTaxBreakdown extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('OrderTaxBreakdown validation error: ID must be populated');
    if (!this.domain) throw new Error('OrderTaxBreakdown validation error: Domain context required');
    return true;
  }
...
module.exports = { OrderTaxBreakdown };
