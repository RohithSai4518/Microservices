/**
 * UserProfile - Domain Model & Invariant Engine
...
class UserProfile extends AggregateRoot {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('UserProfile validation error: ID must be populated');
    if (!this.domain) throw new Error('UserProfile validation error: Domain context required');
    return true;
  }
