/**
 * UserActivityLog - Domain Model & Invariant Engine
...
class UserActivityLog extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('UserActivityLog validation error: ID must be populated');
    if (!this.domain) throw new Error('UserActivityLog validation error: Domain context required');
    return true;
  }
