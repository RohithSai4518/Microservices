/**
 * UserConsentRecord - Domain Model & Invariant Engine
...
class UserConsentRecord extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('UserConsentRecord validation error: ID must be populated');
    if (!this.domain) throw new Error('UserConsentRecord validation error: Domain context required');
    return true;
  }
...
module.exports = { UserConsentRecord };
