/**
 * Team - Domain Model & Invariant Engine
...
class Team extends AggregateRoot {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('Team validation error: ID must be populated');
    if (!this.domain) throw new Error('Team validation error: Domain context required');
    return true;
  }
