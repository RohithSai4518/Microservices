/**
 * UserPreferenceMatrix - Domain Model & Invariant Engine
...
class UserPreferenceMatrix extends AggregateRoot {
  constructor(props = {}) {
    super(props.id || ('use_' + Date.now().toString(36) + '_' + Math.random().toString(36).substring(2, 8)));
    this.domain = 'user';
  ...
  validateInvariants() {
    if (!this.id) throw new Error('UserPreferenceMatrix validation error: ID must be populated');
    if (!this.domain) throw new Error('UserPreferenceMatrix validation error: Domain context required');
    return true;
  }
...
module.exports = { UserPreferenceMatrix };
