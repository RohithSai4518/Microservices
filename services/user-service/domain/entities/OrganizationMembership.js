/**
 * OrganizationMembership - Domain Model & Invariant Engine
...
class OrganizationMembership extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('OrganizationMembership validation error: ID must be populated');
    if (!this.domain) throw new Error('OrganizationMembership validation error: Domain context required');
    return true;
  }
