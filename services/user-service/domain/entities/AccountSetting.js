/**
 * AccountSetting - Domain Model & Invariant Engine
...
class AccountSetting extends AggregateRoot {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('AccountSetting validation error: ID must be populated');
    if (!this.domain) throw new Error('AccountSetting validation error: Domain context required');
    return true;
  }
