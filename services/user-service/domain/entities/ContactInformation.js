/**
 * ContactInformation - Domain Model & Invariant Engine
...
class ContactInformation extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('ContactInformation validation error: ID must be populated');
    if (!this.domain) throw new Error('ContactInformation validation error: Domain context required');
    return true;
  }
