/**
 * UserAddress - Domain Model & Invariant Engine
...
class UserAddress extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('UserAddress validation error: ID must be populated');
    if (!this.domain) throw new Error('UserAddress validation error: Domain context required');
    return true;
  }
