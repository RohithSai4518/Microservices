/**
 * AccountLifecycleService - Application Service
...
class AccountLifecycleService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('user-accountlifecycleservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in AccountLifecycleService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('user.accountlifecycleservice.executed', {
      ...
      this.logger.error('Error executing AccountLifecycleService: ' + err.message, { traceId });
