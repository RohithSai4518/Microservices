/**
 * KycVerificationService - Application Service
...
class KycVerificationService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('user-kycverificationservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in KycVerificationService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('user.kycverificationservice.executed', {
      ...
      this.logger.error('Error executing KycVerificationService: ' + err.message, { traceId });
