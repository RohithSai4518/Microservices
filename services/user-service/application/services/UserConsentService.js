/**
 * UserConsentService - Application Service
...
class UserConsentService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('user-userconsentservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in UserConsentService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('user.userconsentservice.executed', {
      ...
      this.logger.error('Error executing UserConsentService: ' + err.message, { traceId });
