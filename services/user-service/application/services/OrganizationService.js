/**
 * OrganizationService - Application Service
...
class OrganizationService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('user-organizationservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in OrganizationService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('user.organizationservice.executed', {
      ...
      this.logger.error('Error executing OrganizationService: ' + err.message, { traceId });
