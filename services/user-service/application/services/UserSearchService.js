/**
 * UserSearchService - Application Service
...
class UserSearchService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('user-usersearchservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in UserSearchService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('user.usersearchservice.executed', {
      ...
      this.logger.error('Error executing UserSearchService: ' + err.message, { traceId });
