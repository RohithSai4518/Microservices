/**
 * DispatchQueueItem - Domain Model & Invariant Engine
...
class DispatchQueueItem extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('DispatchQueueItem validation error: ID must be populated');
    if (!this.domain) throw new Error('DispatchQueueItem validation error: Domain context required');
    return true;
  }
...
module.exports = { DispatchQueueItem };
