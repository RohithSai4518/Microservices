/**
 * PushSubscriptionToken - Domain Model & Invariant Engine
...
class PushSubscriptionToken extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('PushSubscriptionToken validation error: ID must be populated');
    if (!this.domain) throw new Error('PushSubscriptionToken validation error: Domain context required');
    return true;
  }
...
module.exports = { PushSubscriptionToken };
