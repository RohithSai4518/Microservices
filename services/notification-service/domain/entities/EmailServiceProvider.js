/**
 * EmailServiceProvider - Domain Model & Invariant Engine
...
class EmailServiceProvider extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('EmailServiceProvider validation error: ID must be populated');
    if (!this.domain) throw new Error('EmailServiceProvider validation error: Domain context required');
    return true;
  }
