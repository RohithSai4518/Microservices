/**
 * BatchCampaign - Domain Model & Invariant Engine
...
class BatchCampaign extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('BatchCampaign validation error: ID must be populated');
    if (!this.domain) throw new Error('BatchCampaign validation error: Domain context required');
    return true;
  }
