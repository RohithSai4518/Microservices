/**
 * RecipientProfile - Domain Model & Invariant Engine
...
class RecipientProfile extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('RecipientProfile validation error: ID must be populated');
    if (!this.domain) throw new Error('RecipientProfile validation error: Domain context required');
    return true;
  }
