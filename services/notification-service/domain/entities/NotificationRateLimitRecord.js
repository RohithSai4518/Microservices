/**
 * NotificationRateLimitRecord - Domain Model & Invariant Engine
...
class NotificationRateLimitRecord extends AggregateRoot {
  constructor(props = {}) {
    super(props.id || ('not_' + Date.now().toString(36) + '_' + Math.random().toString(36).substring(2, 8)));
  ...
  validateInvariants() {
    if (!this.id) throw new Error('NotificationRateLimitRecord validation error: ID must be populated');
    if (!this.domain) throw new Error('NotificationRateLimitRecord validation error: Domain context required');
    return true;
  }
...
module.exports = { NotificationRateLimitRecord };
