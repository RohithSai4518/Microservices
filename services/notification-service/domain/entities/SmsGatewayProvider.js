/**
 * SmsGatewayProvider - Domain Model & Invariant Engine
...
class SmsGatewayProvider extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('SmsGatewayProvider validation error: ID must be populated');
    if (!this.domain) throw new Error('SmsGatewayProvider validation error: Domain context required');
    return true;
  }
