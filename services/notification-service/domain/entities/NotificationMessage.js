/**
 * NotificationMessage - Domain Model & Invariant Engine
...
class NotificationMessage extends AggregateRoot {
  constructor(props = {}) {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('NotificationMessage validation error: ID must be populated');
    if (!this.domain) throw new Error('NotificationMessage validation error: Domain context required');
    return true;
  }
