/**
 * NotificationTemplate - Domain Model & Invariant Engine
...
class NotificationTemplate extends AggregateRoot {
  ...
  validateInvariants() {
    if (!this.id) throw new Error('NotificationTemplate validation error: ID must be populated');
    if (!this.domain) throw new Error('NotificationTemplate validation error: Domain context required');
    return true;
  }
