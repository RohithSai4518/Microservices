/**
 * QueueProcessingService - Application Service
...
class QueueProcessingService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('notification-queueprocessingservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in QueueProcessingService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('notification.queueprocessingservice.executed', {
      ...
      this.logger.error('Error executing QueueProcessingService: ' + err.message, { traceId });
