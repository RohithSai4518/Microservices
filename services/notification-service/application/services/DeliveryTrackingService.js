/**
 * DeliveryTrackingService - Application Service
...
class DeliveryTrackingService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('notification-deliverytrackingservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in DeliveryTrackingService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('notification.deliverytrackingservice.executed', {
      ...
      this.logger.error('Error executing DeliveryTrackingService: ' + err.message, { traceId });
