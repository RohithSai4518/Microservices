/**
 * WebhookDispatchService - Application Service
...
class WebhookDispatchService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('notification-webhookdispatchservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in WebhookDispatchService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('notification.webhookdispatchservice.executed', {
      ...
      this.logger.error('Error executing WebhookDispatchService: ' + err.message, { traceId });
