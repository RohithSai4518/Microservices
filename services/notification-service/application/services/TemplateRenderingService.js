/**
 * TemplateRenderingService - Application Service
...
class TemplateRenderingService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('notification-templaterenderingservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in TemplateRenderingService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('notification.templaterenderingservice.executed', {
      ...
      this.logger.error('Error executing TemplateRenderingService: ' + err.message, { traceId });
