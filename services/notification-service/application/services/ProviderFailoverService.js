/**
 * ProviderFailoverService - Application Service
...
class ProviderFailoverService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('notification-providerfailoverservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in ProviderFailoverService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('notification.providerfailoverservice.executed', {
      ...
      this.logger.error('Error executing ProviderFailoverService: ' + err.message, { traceId });
