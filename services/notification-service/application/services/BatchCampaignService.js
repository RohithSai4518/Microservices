/**
 * BatchCampaignService - Application Service
...
class BatchCampaignService {
  constructor(repository, eventBus, options = {}) {
    ...
    this.logger = new Logger('notification-batchcampaignservice');
  ...
  async execute(command, context = {}) {
    ...
    this.logger.info('Executing operation in BatchCampaignService', { traceId, commandName: command ? command.constructor.name : 'DirectCall' });
    ...
    try {
      ...
      if (this.eventBus) {
        await this.eventBus.publish('notification.batchcampaignservice.executed', {
      ...
      this.logger.error('Error executing BatchCampaignService: ' + err.message, { traceId });
...
module.exports = { BatchCampaignService };
